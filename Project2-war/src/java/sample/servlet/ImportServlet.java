...
public class ImportServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Object object = poji.getProduct(id);

            if (object != null) {
                TblProduct product = (TblProduct) object;
                
                HttpSession session = request.getSession();
                session.setAttribute("IMPORT", product);
            }

            RequestDispatcher rd = request.getRequestDispatcher(importPage);
            rd.forward(request, response);
        } catch (NamingException ex) {
            log("ImportServlet _ JNDI " + ex.getMessage());
        } finally {
            out.close();
        }
    }
